...
public class TabUtils {
    ...
    public static Tab get(String id) {
        for (Tab t : list) if (t.getID().equals(id)) return t;
        return new Tab(id);
    }
    ...
    public static void sendTab(Tab tab, User user) throws Exception {
        ...
        for (int i = 0; i < 60; i++) {
            ...
            if (team == null) team = sb.registerNewTeam(center);
    ...
    public static Object[] packets(String[] ss, boolean b) throws Exception {
        ...
        for (int i = 0; i < ss.length; i++) packets[i] = TabUtils.getPacket(ss[i], b, 0);
        return packets;
    }
    ...
    protected static String uniqueField(Tab tab) {
        ...
        String s = "";
        if (!fields.contains(s)) fields.add(s);
